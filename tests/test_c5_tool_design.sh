#!/usr/bin/env bash
# Acceptance test for Task C5 — tool-design pattern files
# Validates that all 5 required pattern files exist with correct frontmatter and body sections.
set -uo pipefail
...
echo "=== Task C5 — Tool Design Pattern Files ==="
echo "Repo: $REPO"
echo ""
...
REQUIRED_FILES=(
    ...
    "search-over-list.md"
    "namespace-tools.md"
    "agents-optimize-tools.md"
    "code-execution-tools.md"
)
...
echo ""
echo "--- Dimensions include tool-design ---"
for f in "${REQUIRED_FILES[@]}"; do
    fp="$PATTERNS/$f"
    [ -f "$fp" ] || continue
    check "$f dimensions include tool-design" \
        "$(grep -q 'tool-design' "$fp" && echo ok || echo fail)"
done
