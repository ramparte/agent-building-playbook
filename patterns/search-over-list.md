...
- Anthropic — https://www.anthropic.com/news/contextual-retrieval — Contextual Retrieval: demonstrates that high-precision retrieval — returning only the relevant results — reduces failed retrievals by up to 49%; the same high-signal-per-token principle this pattern applies to tool output design
- Anthropic, "Writing effective tools for agents — with agents" — https://www.anthropic.com/engineering/writing-tools-for-agents — formalizes: explicitly recommends search-focused tools over list-everything, with filtering via pagination and range selection to conserve context
