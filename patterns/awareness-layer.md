...
- Amplifier — https://github.com/microsoft/amplifier-foundation — the session-analyst agent: a dedicated observer that analyzes session event streams (events.jsonl), diagnoses failures, and searches session history — surfacing patterns the executing agents cannot observe about themselves, from outside their context
