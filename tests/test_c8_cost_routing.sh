#!/usr/bin/env bash
# Acceptance test for Task C8 — cost-routing pattern files
# Validates that all 3 required pattern files exist with correct frontmatter and body sections.
set -uo pipefail
...
echo "=== Task C8 — Cost & Model Routing Pattern Files ==="
echo "Repo: $REPO"
echo ""

# --- List of required files ---
REQUIRED_FILES=(
    "match-model-to-stage.md"
    "role-based-routing.md"
    "delegation-token-economics.md"
)
...
echo ""
echo "--- Dimensions include cost-routing ---"
for f in "${REQUIRED_FILES[@]}"; do
    fp="$PATTERNS/$f"
    [ -f "$fp" ] || continue
    check "$f dimensions include cost-routing" \
        "$(grep -q 'cost-routing' "$fp" && echo ok || echo fail)"
done

echo ""
echo "--- Title checks ---"
check "match-model-to-stage.md title is 'Match Model to Stage'" \
    "$(grep -q '^title:.*Match Model to Stage' "$PATTERNS/match-model-to-stage.md" 2>/dev/null && echo ok || echo fail)"
check "role-based-routing.md title is 'Pull Models From Provisioned Roles'" \
    "$(grep -q '^title:.*Pull Models From Provisioned Roles' "$PATTERNS/role-based-routing.md" 2>/dev/null && echo ok || echo fail)"
check "delegation-token-economics.md title is 'Delegation Is Token Economics'" \
    "$(grep -q '^title:.*Delegation Is Token Economics' "$PATTERNS/delegation-token-economics.md" 2>/dev/null && echo ok || echo fail)"

echo ""
echo "--- Multi-dimension check for delegation-token-economics.md ---"
fp="$PATTERNS/delegation-token-economics.md"
if [ -f "$fp" ]; then
    check "delegation-token-economics.md dimensions include context-engineering" \
        "$(grep -q 'context-engineering' "$fp" && echo ok || echo fail)"
    check "delegation-token-economics.md dimensions include orchestration" \
        "$(grep -q 'orchestration' "$fp" && echo ok || echo fail)"
fi
