#!/usr/bin/env bash
# Acceptance test for Task C6 — orchestration pattern files
# Validates that all 11 required pattern files exist with correct frontmatter and body sections.
set -uo pipefail
...
echo "=== Task C6 — Orchestration Pattern Files ==="
echo "Repo: $REPO"
echo ""

# --- List of required files ---
REQUIRED_FILES=(
    "single-threaded-default.md"
    "share-full-context.md"
    "workflows-vs-agents.md"
    "composable-patterns.md"
    "clean-slate-delegation.md"
    "recon-before-action.md"
    "parallel-independent-tracks.md"
    "architect-then-builder.md"
    "autonomy-when-justified.md"
    "three-primitives.md"
    "guardrails-and-escalation.md"
)
...
echo ""
echo "--- Dimensions include orchestration ---"
for f in "${REQUIRED_FILES[@]}"; do
    fp="$PATTERNS/$f"
    [ -f "$fp" ] || continue
    check "$f dimensions include orchestration" \
