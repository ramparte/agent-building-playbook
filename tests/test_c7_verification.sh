#!/usr/bin/env bash
# Acceptance test for Task C7 — verification pattern files
...
echo "=== Task C7 — Verification Pattern Files ==="
echo "Repo: $REPO"
echo ""
...
REQUIRED_FILES=(
    ...
    "test-what-not-how.md"
    "eval-driven-development.md"
    "standing-eval-capability.md"
    "tests-first-on-api-change.md"
    "three-file-rule.md"
    "evidence-before-assertions.md"
)
...
echo ""
echo "--- Dimensions include verification ---"
for f in "${REQUIRED_FILES[@]}"; do
    fp="$PATTERNS/$f"
    [ -f "$fp" ] || continue
    check "$f dimensions include verification" \
        "$(grep -q 'verification' "$fp" && echo ok || echo fail)"
done
