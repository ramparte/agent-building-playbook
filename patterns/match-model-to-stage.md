...
- 2389 Research, thrifty — https://github.com/2389-research/thrifty — first-party: tiered delegation under the rule "Offload each sprint of work to the weakest model that can do it correctly" — a planner model writes the spec into self-contained sprints, a cheap model executes and self-verifies each against the gate, and the published benchmark reports the same gate quality as the strong model at ~64% lower cost
