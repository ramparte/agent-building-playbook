#!/usr/bin/env bash
# Acceptance test for Task C4 — context-engineering pattern files
# Validates that all 8 required pattern files exist with correct frontmatter and body sections.
set -uo pipefail
...
echo "=== Task C4 — Context Engineering Pattern Files ==="
echo "Repo: $REPO"
echo ""

# --- List of required files ---
REQUIRED_FILES=(
    "context-over-prompt.md"
    "write-select-compress-isolate.md"
    "just-in-time-retrieval.md"
    "checkpoint-handoff-file.md"
    "persist-environment-facts.md"
    "subagents-as-context-sinks.md"
    "thin-pointers.md"
    "long-horizon-memory.md"
)
...
echo ""
echo "--- Dimensions include context-engineering ---"
for f in "${REQUIRED_FILES[@]}"; do
    fp="$PATTERNS/$f"
    [ -f "$fp" ] || continue
    check "$f dimensions include context-engineering" \
